...
def getDatosDNI(ditrict_obj, dni, tipo_busqueda, token):
	try:
		...
		elif tipo_busqueda == 'selenium':
			nombre = servicio_busqueda.get_dni_selenium(dni)
		else:
			nombre = servicio_busqueda.getDatosDNI(dni)
...
def get_data_doc_number(ditrict_obj, tipo_doc, numero_doc, tipo_busqueda, token, format='json'):
	...
	for x in range(0,3):
		if tipo_busqueda == 'apiperu':
			...
			d = servicio_busqueda.get_ruc_selenium(numero_doc)
		else:
			d = servicio_busqueda.getDatosRUC(numero_doc)
		if d['error'] == False:
			break
		elif x == 2:
...
class Partner(models.Model):
	...
	def _doc_number_change(self):
		...
		token = ''
		tipo_busqueda_ruc = 'directa'
		tipo_busqueda_dni = 'directa'
		if self.company_id:
			token = self.company_id.token_apiperu
			tipo_busqueda_ruc = self.company_id.busqueda_ruc
			tipo_busqueda_dni = self.company_id.busqueda_dni
		else:
			token = self.env.company.token_apiperu
			tipo_busqueda_ruc = self.env.company.busqueda_ruc
			tipo_busqueda_dni = self.env.company.busqueda_dni
		if vat and self.l10n_latam_identification_type_id:
			...
			if vat_type == '1':
				...
				response = False
				try:
					if tipo_busqueda_dni == 'apiperu':
						response = servicio_busqueda.get_dni_apiperu(token, vat.strip())
					elif tipo_busqueda_dni == 'selenium':
						response = servicio_busqueda.get_dni_selenium(vat.strip())
					else:
						response = servicio_busqueda.getDatosDNI(vat.strip())
				except Exception:
					reponse = False
				...
				for x in range(0,3):
					if tipo_busqueda_ruc == 'apiperu':
						vals = servicio_busqueda.get_ruc_apiperu(token, vat)
					elif tipo_busqueda_ruc == 'selenium':
						vals = servicio_busqueda.get_ruc_selenium(vat)
					else:
						vals = servicio_busqueda.getDatosRUC(vat)
