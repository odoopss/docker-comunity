...
class PLEReport01(models.Model):
    ...
    cash_line_ids = fields.Many2many(
        'account.move.line', 'cash_accounts_rel', string='Movimientos de efectivo', readonly=True)
    bank_line_ids = fields.Many2many(
        'account.move.line', 'bank_accounts_rel', string='Movimientos de banco', readonly=True)
    ...
    def update_report(self):
        ...
        lines = [
            ...
            ('company_id.partner_id.country_id', '=', self.env.ref(
                'base.pe').id),  # TODO no domiciliados?
            ('date', '>=', str(start)),
            ('date', '<=', str(end)),
            ('parent_state', '=', 'posted'),
        ...
        cash_line_ids = self.env['account.move.line'].search(
            cash_domain, order='date asc')
        bank_line_ids = self.env['account.move.line'].search(
            bank_domain, order='date asc')
    ...
    def generate_report(self):
        ...
        name_01 = self.get_default_filename(
            ple_id='010100', tiene_datos=bool(lines_to_write_01))
        lines_to_write_01.append('')
        txt_string_01 = '\r\n'.join(lines_to_write_01)
        dict_to_write = dict()
        if txt_string_01:
            xlsx_file_base_64 = self._generate_xlsx_base64_bytes(
                lines_to_write_01, name_01[2:], headers=PLE_1_1_HEADER)
        ...
        name_02 = self.get_default_filename(
            ple_id='010200', tiene_datos=bool(lines_to_write_02))
        lines_to_write_02.append('')
        txt_string_02 = '\r\n'.join(lines_to_write_02)
        if txt_string_02:
            xlsx_file_base_64 = self._generate_xlsx_base64_bytes(
                lines_to_write_02, name_02[2:], headers=PLE_1_2_HEADER)
            dict_to_write.update({
